...
def gradient(
    ...
    for i in range(n):
        ei = np.zeros(n)
        ei[i] = 1.0
        print("plus: {}".format(x + delta * ei))
        f1 = func(x + delta * ei, *args, **kwargs)
        f2 = func(x - delta * ei, *args, **kwargs)
        print("f1: {}, f2: {}".format(f1, f2))
        print("f1 - f2: {}".format(f1 - f2))
        print("2 * delta: {}".format(2.0 * delta))
        result[i] = (f1 - f2) / (2.0 * delta)
    return result
...
def hessian(
    ...
    for i in range(n):
        for j in range(n):
            ...
            ei[i] = 1
            ej[j] = 1
            f1 = func(x + delta * ei + delta * ej, *args, **kwargs)
            f2 = func(x + delta * ei - delta * ej, *args, **kwargs)
            f3 = func(x - delta * ei + delta * ej, *args, **kwargs)
            f4 = func(x - delta * ei - delta * ej, *args, **kwargs)
            d = (f1 - f2 - f3 + f4) / (4.0 * delta * delta)
            result[i, j] = d
    return result
...
def cluster(data: list, max_gap: float = 0.05) -> list:
    """Arrange data into groups where successive elements differ by no more than max_gap.
